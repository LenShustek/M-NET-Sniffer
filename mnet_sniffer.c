...
The program appends decoded packets to log.txt as well as the console.
...
16 Apr 2015,  L. Shustek,  v1.2; add "fan" mode; augment help; change program name.
...
char dev_name[80];
FILE *logfile=NULL, *testfile=NULL;
struct _timeb start_time;  // long time; unsigned short millitm;
bool did_newline = true;
static char blanks[]="                                                                       ";

void SayUsage (char *programname) {
    printf("Invoke as \"mnet_sniffer [-un]\"\n");
    printf("optional: -un  ignores packets not to or from unit n\n");
    printf("This version operates as follows:\n");
#if MNET
    printf(" read MNET traffic as RS232 data on COM%d\n", mnet_com_port);
#endif
#if COOLMASTER
    printf(" read CoolMaster traffic as RS232 data on COM%d\n", coolmaster_com_port);
 #if COOLMASTER_KB
    printf(" simulate CoolMaster traffic from keyboard using a loopback plug\n");l
  #endif
#endif
    printf(" write decoded packets to the console and append to log.txt\n");
}
...
void getmode_ack (void) {
    int parm = packet.data[2];
    output(parm==7?" heat":parm==8?" cool":parm==0x0d?" fan only":"???");
}
...
int main(int argc,char *argv[]){
    ...
    printf("Mitsubishi M-NET Sniffer, version " VERSION ", /h for help\n");
